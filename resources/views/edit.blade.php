...
<form action="{{ route('contacts.update', [ 'contact' => $contact->userId ])}}" method="POST"  enctype="multipart/form-data">
...
<input type="file" name="cover_image"  value = "{{$contact->cover_image}}"  class="form-control">
@error('cover_image')
<span class="text-danger">{{$message}}</span>
@enderror
